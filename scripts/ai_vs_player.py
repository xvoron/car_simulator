...
from scripts.ai import DQN_agent
WIDTH = 1600
HEIGHT = 800
    ...
    def run(self):
        ...
        while  not self.reset:
            ...
            while not self.exit:
                ...
                """
                if self.with_track:
                    label_score = self.my_font.render("Scores: p:{0},\n ai:{1}".format(self.car_player.score,self.car_ai.score),1,(0,255,0))
                ...
                if not self.done_player:
                    ...
                    print("[INFO] Draw a player car")
                    self.car_player.draw(self.screen)
                if not self.done_ai:
                    self.car_ai.update(dt)
                    print("[INFO] Draw an ai car")
